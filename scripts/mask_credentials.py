...
def find_repo_root() -> Path:
    ...
    while cur != cur.parent:
        if (cur / "config.yaml").exists() and ((cur / "README.md").exists() or (cur / "README_CN.md").exists()):
            return cur
        cur = cur.parent
    return Path.cwd()
...
def main():
    ...
    backup_file = root_dir / ".credentials.backup"
    ...
    if args.restore:
        ...
        if (not target_project_id or not target_app_id) and backup_file.exists():
            try:
                import json
                b_data = json.loads(backup_file.read_text(encoding="utf-8"))
                target_project_id = target_project_id or b_data.get("project_id")
                target_app_id = target_app_id or b_data.get("ge_app_id")
                print(f"ℹ️ Loaded credentials from {backup_file.name}")
            except Exception as e:
                print(f"⚠️ Failed to read {backup_file.name}: {e}")

        if not target_project_id or not target_app_id:
        ...
        mods = restore_files(root_dir, target_project_id, target_app_id, dry_run=args.dry_run)
    ...
    if not args.dry_run and (project_id or ge_app_id):
        try:
            import json
            backup_file.write_text(json.dumps({"project_id": project_id, "ge_app_id": ge_app_id}, indent=2), encoding="utf-8")
            print(f"💾 Saved local credentials cache to {backup_file.name} (gitignored)")
        except Exception as e:
            print(f"⚠️ Could not write {backup_file.name}: {e}")
