...
在实时竞价（Real-Time Bidding, RTB）广告投放中，广告主通过 DSP 平台参与毫秒级的二阶密封拍卖（Generalized Second Price Auction）。广告主通常设定固定的预算周期（如单日预算 $B$）以及期望达成的目标转化成本（如单次点击成本上限 $\text{Target\_CPC}$）。

因此，自动出价代理（Auto-Bidding Agent）的核心优化任务在数学上是一个**带严格不等式约束的随机动态运筹优化问题（Constrained Stochastic Optimization Problem）**：

$$\max_{\pi} \sum_{t=1}^{T} \text{Click}_t(b_t)$$

$$\text{s.t.} \quad \sum_{t=1}^{T} \text{Cost}_t(b_t) \le B \quad \text{（全局预算硬约束）}$$

$$\frac{\sum_{t=1}^{T} \text{Cost}_t(b_t)}{\sum_{t=1}^{T} \text{Click}_t(b_t)} \le \text{Target\_CPC} \quad \text{（目标成本硬约束）}$$

$$b_t \in \text{Support}(\mathcal{D}_{\text{history}}) \quad \text{（数据有效性与分布可信约束）}$$

其中：
* $\pi$ 为待演化的自适应出价乘子策略函数 `get_multiplier(state) -> float`；
* $b_t = \text{Target\_CPC} \times p\text{CTR}_t \times 1000 \times m_t$ 为实际竞价请求时的出价（以 CPM 为单位）；
* $\text{Click}_t \in \{0, 1\}$，$\text{Cost}_t$ 为第二名出价出清价格。

---

## 二、 适应度得分（Fitness Score）的完整计算流程

评估过程完全在沙盒沙箱中由 [`src/evaluate.py`](file:///Users/ianychen/Projects/ads-AE/src/evaluate.py) 与 [`src/program.py`](file:///Users/ianychen/Projects/ads-AE/src/program.py) 自动化驱动，全过程分为三个阶段：
...
* **物理状态构建（`BidState`）**：在每个竞价请求 $t$，提取广告主侧真实可观测的物理特征：
  * **请求级特征**：当前预估点击率 $p\text{CTR}_t$、全盘历史平均点击率 $\overline{p\text{CTR}}$；
  * **宏观进度特征**：剩余预算比例 $\text{rem\_budget\_ratio} \in [0, 1]$、时间进度比例 $\text{time\_progress\_ratio} \in [0, 1]$；
  * **消耗反馈特征**：消耗速率比值 $\text{spend\_velocity} = \frac{\text{实际消耗}}{\text{理想线性消耗}}$；
  * **成本控制特征**：累计点击成本 $\text{current\_cpc}$、成本比例 $\text{cpc\_ratio} = \frac{\text{current\_cpc}}{\text{Target\_CPC}}$；
  * **滑动窗口短期统计（$W=100$）**：最近竞价胜率 $\text{recent\_win\_rate}$、最近成交均价 $\text{recent\_cpc}$；
  * **上一周期动作**：上一轮出价乘子 $\text{last\_multiplier}$。

### 2. 市场响应与二阶清算结算
候选策略代码 `get_multiplier(state)` 计算出乘子 $m_t \in [0.5, 2.0]$，折算为单次出价 $b_t$。随后通过离线拟合的买方市场响应模型评估：
* **胜标概率**：基于 Kaplan-Meier 乘积限生存分析模型（考虑右删失）预测 $P(\text{win} \mid b_t)$；
* **清算成本**：二阶定价期望扣费 $E[\text{cost} \mid \text{win}, b_t]$；
* **期望新增消耗**：$\Delta \text{Spend} = \frac{E[\text{cost}]}{1000} \times P(\text{win} \mid b_t)$；
* **期望新增点击**：$\Delta \text{Clicks} = p\text{CTR}_t \times P(\text{win} \mid b_t)$；
...
$$\text{Fitness} = 
\begin{cases} 
-1000.0 - (\text{cum\_spend} - B) & \text{若 } \text{cum\_spend} > 1.02 \times B \quad \text{【预算超支淘汰】} \\
-500.0 - 1000.0 \times \frac{\text{final\_cpc} - \text{Target\_CPC}}{\text{Target\_CPC}} & \text{若 } \text{final\_cpc} > 1.01 \times \text{Target\_CPC} \quad \text{【CPC超标淘汰】} \\
-200.0 - 500.0 \times \text{ood\_ratio} & \text{若 } \text{ood\_ratio} > 0.08 \quad \text{【OOD越界淘汰】} \\
\text{cum\_clicks} - 5.0 \times \frac{\text{Target\_CPC} - \text{final\_cpc}}{\text{Target\_CPC}} & \text{其它（所有硬约束合规）} \quad \text{【合格策略得分】}
\end{cases}$$
...
* **松弛度项的精妙调节**：
  $$\text{cpc\_slack} = \frac{\text{Target\_CPC} - \text{final\_cpc}}{\text{Target\_CPC}}$$
  公式中引入 `- 5.0 * cpc_slack`，相当于在经济学上引入了**“资本闲置机会成本”**：
...
| :--- | :--- | :--- | :--- | :--- |
| **方案 A：朴素点击量目标** | $\text{Fitness} = \text{cum\_clicks}$ | 策略无脑顶格提价，利用模拟器极值盲区刷分 | 预算数小时烧空，实际 CPC 严重超标（>300 RMB），线上不可用 | ❌ **严重不可行** |
| **方案 B：仅成本达标目标** | $\text{Fitness} = - \text{abs}(\text{final\_cpc} - \text{Target})$ | 策略过度保守，只买极低成本流量 | 预算消耗不足 10%（严重欠量），点击数极低，商业目标破产 | ❌ **业务不可用** |
...
如需在代码中查阅或复现具体逻辑，可参考以下文件位置：

1. **核心适应度计算实现**：
   * 仿真与分段惩罚：[`src/program.py` (第 85-214 行)](file:///Users/ianychen/Projects/ads-AE/src/program.py#L85-L214)
   * 适应度函数封装与诊断信息输出：[`src/evaluate.py` (第 134-258 行)](file:///Users/ianychen/Projects/ads-AE/src/evaluate.py#L134-L258)
2. **自动化测试与断言验证**：
   * 候选策略执行与异常打分用例：[`tests/test_evolution.py`](file:///Users/ianychen/Projects/ads-AE/tests/test_evolution.py)
   * 出价状态与乘子边界用例：[`tests/test_bidding.py`](file:///Users/ianychen/Projects/ads-AE/tests/test_bidding.py)
3. **真实测试集 Oracle 验证实现**：
   * 二阶出清与基准对比：[`src/bidding/benchmark.py`](file:///Users/ianychen/Projects/ads-AE/src/bidding/benchmark.py)
   * 自动化报表渲染（含 Gemini 机理剖析）：[`src/report.py`](file:///Users/ianychen/Projects/ads-AE/src/report.py)
